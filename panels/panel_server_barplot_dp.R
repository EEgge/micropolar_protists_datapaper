...
output$bp_text <- renderText({
  barplot()$caption
})

barplot <- eventReactive(input$actionb_barplot, { 
  ...
  if (input$which_tab == "merged") {
    asvtab_bp_start <- asvtab3b_merged_subsamp_prop
  } else {
    asvtab_bp_start <- asvtab1b_nonmerged_prop
  }
  ...
  bp_reads <- ggplot(taxlevel_pivot_wmeta4, aes(x=reorder(station_depth, desc(station_depth)), y=value, fill = Taxonomic_group, 
  ...
  bp_reads_ly <- ggplotly(bp_reads, tooltip = "text")
  ...
  bp_reads <- ggplot(taxlevel_pivot_wmeta4, aes(x=reorder(station_dep_com, desc(station_dep_com)), y=value, fill = Taxonomic_group, 
  ...
  return(list(bp_reads_ly=bp_reads_ly, caption = caption))
})
