...
minmaxtable <- reactive({
  ...
  nASVs_taxlevel <- asvtab3b_merged_subsamp_prop %>% group_by(.data[[input$taxlevel_tableS2]]) %>% count()
  
  roundfun <- function (x) {round(x,0)}
  
  #Min-max per division #
  asvtabprop_long <- pivot_longer(asvtab3b_merged_subsamp_prop, cols =!all_of(taxlevels),  names_to = "sample_sizefract")
